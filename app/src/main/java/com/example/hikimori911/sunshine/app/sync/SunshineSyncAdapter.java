...
import com.example.hikimori911.sunshine.app.MainActivity;
import com.example.hikimori911.sunshine.app.R;
...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    private void notifyWeather() {
        ...
        String notificationsKey = context.getString(R.string.pref_notifications_enabled);
        boolean notificationsEnabled = prefs.getBoolean(notificationsKey,true);

        if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS && notificationsEnabled) {
    ...
    long addLocation(String locationSetting, String cityName, double lat, double lon) {
        ...
        Cursor cursor = ctx.getContentResolver()
                .query(WeatherContract.LocationEntry.CONTENT_URI,
                        ...
                        new String[]{cityName, String.valueOf(lon), String.valueOf(lat), locationSetting}, null);

        if(cursor.moveToNext()){
            id = cursor.getLong(cursor.getColumnIndex(WeatherContract.LocationEntry._ID));
        }else{
            //there is no same location in db
            ContentValues values = new ContentValues();
            values.put(WeatherContract.LocationEntry.COLUMN_CITY_NAME,cityName);
            values.put(WeatherContract.LocationEntry.COLUMN_COORD_LAT,lat);
            values.put(WeatherContract.LocationEntry.COLUMN_COORD_LONG,lon);
            values.put(WeatherContract.LocationEntry.COLUMN_LOCATION_SETTING,locationSetting);
            id = ContentUris.parseId(ctx.getContentResolver().insert(WeatherContract.LocationEntry.CONTENT_URI, values));
        }
        cursor.close();
        return id;
    }
